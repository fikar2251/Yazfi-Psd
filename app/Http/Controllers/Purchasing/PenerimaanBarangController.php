...
class PenerimaanBarangController extends Controller
{
    ...
    public function create(Purchase $penerimaan, Request $request)
    {
        ...
        // dd($tukar);
        return view('purchasing.penerimaan-barang.create', compact('ppn', 'purchases', 'purchase', 'nourut', 'status_barang', 'inout', 'penerimaan', 'gudang', 'ppn_partial', 'warehouses'));
    ...
    public function store(StorePenerimaanRequest $request)
    {
        ...
        if ($status_barang == 'pending') {
            ...
            $harta = DB::table('new_chart_of_account')->where('id', 17)->select('balance')->first();
            $persediaan = DB::table('new_chart_of_account')->where('id', 11)->select('balance')->first();
            $total = $request->grandtotal;

            $transaction = [
                ['chart_id' => 17,
                    'no_transaksi' => $request->no_penerimaan_barang,
                    ...
                    'credit' => '',
                    'debit' => $total,
                    'last_balance' => $harta->balance - $total,
                    'template_id' => 19,
                    'is_active' => 1,
                ...
                ['chart_id' => 11,
                    ...
                    'credit' => $total,
                    'debit' => '',
                    'last_balance' => $persediaan->balance + $total,
                    'template_id' => 20,
                    'is_active' => 1,
                ],
            ];
            DB::table('transactions')->insert($transaction);
            DB::table('new_chart_of_account')->where('id', 17)->update([
                'balance' => $harta->balance - $total,
            ]);
            DB::table('new_chart_of_account')->where('id', 11)->update([
                'balance' => $persediaan->balance + $total,
            ]);
            DB::commit();
