...
use App\Purchase;
use App\Reinburst;
use App\PembatalanUnt;
use App\Spr;
use App\TukarFaktur;

class AjaxController extends Controller
{
    public function GetCancelingPieChartData()
    {
        $agama = DB::table('pembatalan_unit')->get();
        $resource = [];
        foreach ($agama as $data) {
            array_push(
                $resource,
                [
                    'label' => $data->no_pembatalan,
                    // 'data' => [
    ...
    public function ajax_pengajuan(Request $request){
        ...
        if(request()->ajax()){
            ...
            if(!empty($request->from))
            {
                ...
                ->whereBetween('pengajuans.tanggal_pengajuan', array($request->from, $request->to))
